...
- (void)openInInAppBrowser:(NSURL*)url withOptions:(NSString*)options
{
    ...
    if (self.inAppBrowserViewController == nil) {
        ...
        self.inAppBrowserViewController.automaticallyAdjustsScrollViewInsets = false;
...
- (void)show:(CDVInvokedUrlCommand*)command
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (weakSelf.inAppBrowserViewController != nil) {
            ...
            frame.size.height = frame.size.height - statusBarHeight - 44;
            frame.origin.y = statusBarHeight + 44;
...
{
    if (self.inAppBrowserViewController == nil) {
        NSLog(@"Tried to back IAB after it was closed.");
        return;
    }
    __weak CDVInAppBrowser* weakSelf = self;
    // Run later to avoid the "took a long time" log message.
    dispatch_async(dispatch_get_main_queue(), ^{
        if (weakSelf.inAppBrowserViewController != nil && weakSelf.inAppBrowserViewController.webView.canGoBack) {
            [weakSelf.inAppBrowserViewController.webView goBack];
        }
    });
}
...
- (BOOL)webView:(UIWebView*)theWebView shouldStartLoadWithRequest:(NSURLRequest*)request navigationType:(UIWebViewNavigationType)navigationType
{
   ...
   if(self.callbackId != nil){
       ...
       CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                                                     messageAsDictionary:@{@"type":@"browse", @"url":url, @"status":@100}];
       [pluginResult setKeepCallback:[NSNumber numberWithBool:YES]];
       [self.commandDelegate sendPluginResult:pluginResult callbackId:self.callbackId];
   }
   return YES;
...
{
    return _previousStatusBarStyle;
}
...
- (void)viewWillAppear:(BOOL)animated
{
    if (IsAtLeastiOSVersion(@"7.0")) {
        ...
        if ([statusBar respondsToSelector:@selector(setBackgroundColor:)]) {
            statusBar.backgroundColor = _previousStatusBarStyle == 0 ? [UIColor whiteColor] : [UIColor blackColor];
        }
    }
...
- (void) rePositionViews {
    ...
    CGRect frame = [[UIScreen mainScreen] bounds];
    frame.size.height = frame.size.height - 2*statusBarHeight ;
